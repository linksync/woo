...
?>">
            ...
            <table class="form-table">
                <tbody>
                            ...
                            foreach ($registers['registers'] as $register) {
                                ...
                                foreach ($users['users'] as $user) {
                                    ...
                                    echo ( $userDb == $user['id'] . '|' . $user['username'] ? 'checked' : '');
                                                    ...
                                                    ?>
                                        <th  class="titledesc">Outlets<a href="https://www.linksync.com/help/woocommerce"><img title="If you have multiple Outlets in Vend, use this option to select which Outlet you want orders from WooCommerce to be associated with. If you have multiple registers and users for an outlet, you can also choose which register and/or user you want the orders to be imported against. " style="margin-bottom: -4px;margin-left: 4px;" src="../wp-content/plugins/linksync/img/help.png" height="16" width="16"></a></th>
                    ...
                    <tr valign="top">
                        <th  class="titledesc">Tax mapping<a href="https://www.linksync.com/help/woocommerce"><img title="When syncing orders, both Vend and WooCommerce have their own tax configurations - use these Tax Mapping settings to 'map' the Vend taxes with those in your WooCommerce store." style="margin-bottom: -4px;margin-left: 4px;" src="../wp-content/plugins/linksync/img/help.png" height="16" width="16"></a></th>
                        <td class="forminp forminp-checkbox"><?php
                            #Check for the Taxes 
                            ...
                            foreach ($implode_tax['tax_name'] as $tax_classes_name => $woo_taxes) {
                                global $wpdb;
                                $tax_rates = mysql_query("SELECT * FROM `" . $wpdb->prefix . "woocommerce_tax_rates` WHERE `tax_rate_class` ='" . $woo_taxes . "'");
                                if (mysql_num_rows($tax_rates) != 0) {
                                                ?> <li><b><?php echo ucfirst($tax_classes_name); ?></b> </li> <?php
                                while ($tax_rate = mysql_fetch_assoc($tax_rates)) {
                                                    ?>
...
#Check for the Taxes 
                ...
                <tr valign="top">
                    ...
                    <td class="forminp forminp-checkbox">
                <legend style="display: inline-block;width: 25em; ">  <input type="radio"  checked  <?php echo (get_option('vend_to_wc_customer') == 'customer_data' ? 'checked' : ''); ?> value="customer_data" name="vend_to_wc_customer" />Import Customer data<a href="https://www.linksync.com/help/woocommerce"><img title="Select this option if you'd like customer data, such as name, email address and shipping and billing address, to be included when importing orders from Vend."   style="margin-left: 4px;margin-bottom: -3px;"src="../wp-content/plugins/linksync/img/help.png" height="16" width="16"></a></legend><br>
                <legend style="display: inline-block;width: 25em; "><input type="radio" <?php echo (get_option('vend_to_wc_customer') == 'guest' ? 'checked' : ''); ?> value="guest" name="vend_to_wc_customer" />Import as 'Guest'<a href="https://www.linksync.com/help/woocommerce"><img title="Select this option if you're not interested in including the customer information when importing orders from Vend." style="margin-left: 4px;margin-bottom: -3px;"  src="../wp-content/plugins/linksync/img/help.png" height="16" width="16"></a><br/><br/>
                </legend>
                </td>
                </tr>
                </tbody>
...
<script>
    ...
    linksync_jQuery(document).ready(function() {
        linksync_jQuery("input[name='order_sync_type']").click(function() {
            if (linksync_jQuery("#disabled_sync_id").is(":checked")) {
                linksync_jQuery('#order_sync_setting').slideUp(500);
            } else if (linksync_jQuery("#wc_to_vend_sync_id").is(":checked")) {
                linksync_jQuery('#order_sync_wc_to_vend').slideDown(500);
                linksync_jQuery('#order_sync_vend_to_wc').slideUp(500);
                linksync_jQuery('#order_sync_setting').slideDown(500);
            } else if (linksync_jQuery("#vend_to_wc_sync_id").is(":checked")) {
                linksync_jQuery('#order_sync_vend_to_wc').slideDown(500);
                linksync_jQuery('#order_sync_wc_to_vend').slideUp(500);
                linksync_jQuery('#order_sync_setting').slideDown(500);
            } else {
                linksync_jQuery('#order_sync_setting').slideDown(500);
                linksync_jQuery('#order_sync_wc_to_vend').slideDown(500);
        ...
        if (linksync_jQuery(".outlet_class").is(":checked")) {

<?php if (!empty($outletDb)) { ?>
                linksync_jQuery(".check").hide();
                linksync_jQuery("#" + "<?php echo $outletDb; ?>").fadeIn(500);
<?php } else { ?>
                var outlet = linksync_jQuery(".outlet_class").val();
                linksync_jQuery(".check").fadeOut(500);
                linksync_jQuery("#" + outlet).fadeIn(500);
<?php } ?>
        }
        linksync_jQuery('.outlet_class').click(function() {
